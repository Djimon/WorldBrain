...
- **D25 — Ganze-App-Transformation via Top-Bar-Toggle (2026-08-16 neugefasst, ersetzt die frühere „Content-Pane-Swap"-Fassung).** Ein **globaler, immer sichtbarer Umschalter in der Top-Bar** („Bearbeiten ⟷ Spielen") transformiert die **gesamte App-Shell** — nicht nur den Hauptbereich. Gilt vorerst **nur für die DM-App** (eine eigene Player-.exe existiert noch **nicht** → später; dann bootet die Player-App direkt read-only ohne Toggle).
  ...
  - **Verhältnis zu D23 (reconciled 2026-08-30, #365):** Der Toggle selbst ist **kein** Daten-Layer-Schalter — was das **Edit-Ziel** bestimmt, ist der **aktive Campaign-Kontext**, nicht der Modus-Toggle. Konkret: **Bearbeiten (Autor)** hat keinen aktiven Campaign-Kontext → Edits treffen die **Welt-Basis**. **Spielen als DM** setzt beim Eintritt eine aktive Campaign (`activeSessionId`) → Entity-/Event-Edits landen **campaign-scoped als Override** (D23-Default), Basis-Welt unberührt; der **Promote-Schalter** (S21/#365) hebt einen Override opt-in in die Welt-Basis. **Quelle des Edit-Ziels:** `useAppMode()` — `campaignId = activeSessionId`, wenn `mode==='play' && sessionRole==='dm'`; sonst `undefined` (Welt-Basis). Nur der **DM/Host** editiert (hat `database`, D29); Player sind read-only (S23) und erreichen den Promote-Schalter nie. **Korrektur:** die frühere Formulierung „Spielen(DM) editiert die Welt-Basis" war zu weit gefasst — sonst wäre die gesamte Override-/Promote-Ebene (S20/S21, p0) unerreichbar. D25s Kernaussage bleibt wahr: nicht der Toggle schaltet den Layer, sondern der Campaign-Kontext.
...
## Nachschärfung 2026-08-31 (#387 Join-Handshake) — Decision 32

Umsetzung von #387 (DB-loser Join/Auth als Transport-Handshake) erzwang zwei Präzisierungen der Transport-Schicht (S01 #350 / S11 #367), die über den ursprünglichen AC hinausgehen. **Bewusst so — nicht als Regression zurückbauen.**

- **D32 — Transport-Präzisierungen fürs Join-Handshake (#387).**
  - **(a) Pre-auth-Ausnahme im Decision-8-Token-Gate.** Der Beitritt ist per Definition pre-auth: der Spieler hat beim `join_request` (bzw. `reconnect_request`) noch/wieder kein gültiges Player-Token und kann das per-Nachricht Token-Gate (Decision 8, `webrtc-transport.ts` `authenticate`) nicht passieren. Diese **zwei Handshake-Typen sind vom Gate ausgenommen** (`PRE_AUTH_MESSAGE_TYPES` in `session-transport.ts`) und werden **host-autoritativ von `host-join-sync` validiert** (Code gegen `invite_codes`, Token gegen `session_players`). **Alle anderen Nachrichten bleiben voll gegated** — Decision 8 bleibt für allen Spielverkehr intakt, secure-by-default. Analogie: ein Handshake liegt unter der Auth-Ebene (wie TLS-Handshake vor dem verschlüsselten Kanal, Login-Request vor dem Session-Token).
  - **(b) Buffer-until-open (`onConnected`-Semantik präzisiert).** `attachSignaling`s `onConnected` feuert beim **Broker-Rendezvous** — **VOR** der P2P-DataChannel-Öffnung. Ein proaktiver früher `send()` (z.B. `join_request`) darf deshalb **nicht werfen**: `WebRtcTransport.send()` **puffert** bei noch geschlossenem Kanal in eine bounded Outbox und **flusht bei `onopen`** in Reihenfolge. Damit ist der `SessionTransport`-Vertrag (`send(): Promise<void>` = „wird zugestellt") ehrlich, und **kein** Sender (Join, Token-Moves, künftig Würfeln/Bogen) muss je Channel-Readiness kennen. Der frühere `throw new Error('Transport not connected')` in `send()` ist damit **entfernt** (Regressions-Guard in `tests/m10-s01-webrtc-transport.test.ts`).
  - **Foundation-Bezug:** Der Signaling-Adapter-Vertrag aus dem #380-Spike (`src/services/signaling/`) bleibt unangetastet — beide Präzisierungen sitzen in der Transport-Schicht **darüber**, nicht im Adapter. Der Handshake-Roundtrip selbst ist minimal und unvermeidbar: nur der Host hat `invite_codes` und stellt das Token aus, das der DB-lose Client für spätere Auth braucht.
