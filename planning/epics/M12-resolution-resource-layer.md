...
12. **Jede Deklaration ist per stabiler ID adressierbar und überschreibbar.** Jedes Resolution-/Resource-Ding (Roll-Target, Band-Set, Ressource, Transition, Hook, Modifikator) trägt eine stabile ID (z.B. `transition:long_rest`, `bands:attack`, `resource:hp`). Das ist die Voraussetzung dafür, dass Hausregeln (EPIC-019 House-Rule Overlays) *einzelne* Regeln übersteuern können, ohne das System-Plugin zu forken. **Jetzt mitziehen — nachrüsten ist teuer.** Gilt spiegelbildlich für die M9-Deklarationen (Formeln, Tabellen, Feld-Kategorien).
