...
describe('M8-S02 session list & management', () => {
  describe('session list display', () => {
    it('shows session titles', () => {
      mockListSessions.mockReturnValue(SAMPLE_SESSIONS);
      render(<SessionList projectId="proj-1" projectDir="/p" onResumeSession={vi.fn()} />);
      expect(screen.getByText(/Der erste Abend/i)).toBeInTheDocument();
      expect(screen.getByText(/Zweite Session/i)).toBeInTheDocument();
    });

    it('shows created date for each session', () => {
      mockListSessions.mockReturnValue(SAMPLE_SESSIONS);
      render(<SessionList projectId="proj-1" projectDir="/p" onResumeSession={vi.fn()} />);
      expect(screen.getByText(/2026-01-10|Jan.*2026|10\.01\.2026/i)).toBeInTheDocument();
    });

    it('shows last_active_at for each session', () => {
      mockListSessions.mockReturnValue(SAMPLE_SESSIONS);
      render(<SessionList projectId="proj-1" projectDir="/p" onResumeSession={vi.fn()} />);
      expect(screen.getByText(/23:30|2026-01-10T23/i)).toBeInTheDocument();
    });

    it('shows calendar_position when set', () => {
      mockListSessions.mockReturnValue(SAMPLE_SESSIONS);
      render(<SessionList projectId="proj-1" projectDir="/p" onResumeSession={vi.fn()} />);
      expect(screen.getByText(/Jahr 1432/i)).toBeInTheDocument();
    });

    it('shows system plugin name when set', () => {
      mockListSessions.mockReturnValue(SAMPLE_SESSIONS);
      render(<SessionList projectId="proj-1" projectDir="/p" onResumeSession={vi.fn()} />);
      expect(screen.getByText(/dnd5e-srd/i)).toBeInTheDocument();
    });

    it('shows empty state message when no sessions exist', () => {
      mockListSessions.mockReturnValue([]);
      render(<SessionList projectId="proj-1" projectDir="/p" onResumeSession={vi.fn()} />);
      expect(screen.getByText(/keine session|no session/i)).toBeInTheDocument();
    });
  });

  describe('resume session', () => {
    it('each session has a resume / fortsetzen button', () => {
      mockListSessions.mockReturnValue(SAMPLE_SESSIONS);
      render(<SessionList projectId="proj-1" projectDir="/p" onResumeSession={vi.fn()} />);
      const buttons = screen.getAllByRole('button', { name: /fortsetzen|resume/i });
      expect(buttons.length).toBeGreaterThanOrEqual(1);
    });

    it('clicking resume calls onResumeSession with session id', () => {
      const onResume = vi.fn();
      mockListSessions.mockReturnValue([SAMPLE_SESSIONS[0]]);
  ...
  describe('new session form', () => {
    ...
    it('submitting with title calls createSession with title and projectId', () => {
      mockListSessions.mockReturnValue([]);
      mockCreateSession.mockReturnValue({ id: 'new-s', title: 'Night 3', archived: false });
      render(<SessionList projectId="proj-1" projectDir="/p" onResumeSession={vi.fn()} />);
      fireEvent.click(screen.getByRole('button', { name: /neue session/i }));
      fireEvent.change(screen.getByRole('textbox', { name: /titel|title/i }), { target: { value: 'Night 3' } });
      fireEvent.click(screen.getByRole('button', { name: /erstellen|anlegen|create/i }));
      expect(mockCreateSession).toHaveBeenCalledWith(expect.objectContaining({ title: 'Night 3', projectId: 'proj-1' }));
    });
  ...
  describe('archive', () => {
    it('each session has an archive / archivieren button', () => {
      mockListSessions.mockReturnValue([SAMPLE_SESSIONS[0]]);
      render(<SessionList projectId="proj-1" projectDir="/p" onResumeSession={vi.fn()} />);
      expect(screen.getByRole('button', { name: /archiv/i })).toBeInTheDocument();
    });

    it('clicking archive calls archiveSession with session id', () => {
      mockArchiveSession.mockClear();
      mockListSessions.mockReturnValue([SAMPLE_SESSIONS[0]]);
      render(<SessionList projectId="proj-1" projectDir="/p" onResumeSession={vi.fn()} />);
      fireEvent.click(screen.getByRole('button', { name: /archiv/i }));
      expect(mockArchiveSession).toHaveBeenCalledWith(expect.objectContaining({ sessionId: 's1' }));
    });
  });
  ...
  describe('archive view', () => {
    ...
    it('archive view shows archived sessions', () => {
      mockListSessions.mockImplementation(({ includeArchived }) =>
  ...
  describe('no prompt/alert/confirm', () => {
    it('SessionList.tsx does not use window.prompt, alert or confirm', async () => {
      const src = await import('fs').then(fs => fs.readFileSync('src/ui/SessionList.tsx', 'utf-8'));
